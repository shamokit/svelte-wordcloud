...
`WordCloud3D` renders words across multiple depth layers using WebGL (Three.js via Threlte). Depth layers are navigated with the built-in depth scrollbar or pinch gesture. Zoom is controlled with `Ctrl + Wheel` or the built-in zoom scrollbar.
...
| `wheelScrollSpeed` | `number` | `1` | Multiplier for pinch depth-scroll speed. |
