...
`WordCloud3D` renders words across multiple depth layers using WebGL (Three.js via Threlte). The viewer can navigate layers with `Ctrl + Wheel`, pinch gesture, or the built-in depth scrollbar. The scroll range extends slightly beyond the first and last layer so words near the edges can be inspected.
